...
name: webapp-testing
name_chn: Web 应用测试
description: 使用 Playwright 与本地 Web 应用程序交互和测试的工具包。支持验证前端功能、调试 UI 行为、捕获浏览器截图以及查看浏览器日志。
...
**始终先使用 `--help` 运行脚本**以查看用法。在尝试运行脚本并发现确实需要定制解决方案之前,不要读取源代码。这些脚本可能非常大,因此会污染上下文窗口。它们的存在是为了作为黑盒脚本直接调用,而不是被摄取到上下文窗口中。
...
with sync_playwright() as p:
    ...
    page.goto('http://localhost:5173') # 服务器已经在运行并准备就绪
...
- **将捆绑脚本用作黑盒** - 要完成任务,考虑 `scripts/` 中可用的脚本是否可以提供帮助。这些脚本可靠地处理常见、复杂的工作流程,而不会使上下文窗口混乱。使用 `--help` 查看用法,然后直接调用。
