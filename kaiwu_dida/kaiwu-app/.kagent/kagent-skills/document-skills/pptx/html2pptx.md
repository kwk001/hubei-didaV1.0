...
- `<i>`, `<em>` - 斜体文本（内联格式化）
- `<u>` - 下划线文本（内联格式化）
- `<span>` - 带 CSS 样式的内联格式化（粗体、斜体、下划线、颜色）
- `<br>` - 换行
- `<div>` 带 bg/border - 变成形状
- `<img>` - 图像
...
**只使用普遍可用的 web-safe 字体：**
- ✅ Web-safe 字体：`Arial`, `Helvetica`, `Times New Roman`, `Georgia`, `Courier New`, `Verdana`, `Tahoma`, `Trebuchet MS`, `Impact`, `Comic Sans MS`
...
- 内联格式化：使用 `<b>`, `<i>`, `<u>` 标签或带 CSS 样式的 `<span>`
...
- **文本对齐**：在需要时使用 CSS `text-align`（`center`、`right` 等）作为 PptxGenJS 文本格式化的提示，如果文本长度略有偏差
...
- **盒阴影**：`<div>` 元素上的 CSS `box-shadow` 转换为 PowerPoint 阴影
  ...
  - 注意：PowerPoint 不支持内/内阴影，将被跳过

### 图标和渐变

- **关键：永远不要使用 CSS 渐变（`linear-gradient`, `radial-gradient`）** - 它们不会转换为 PowerPoint
...
2. **内容不得溢出 body** - 报告溢出及精确测量值
3. **CSS 渐变** - 报告不支持的渐变使用
4. **文本元素样式** - 报告文本元素上的背景/边框/阴影（仅允许在 div 上）

**所有验证错误都被收集并一起报告**在单个错误消息中，允许你一次修复所有问题而不是一次一个。
...
使用 `html2pptx` 将 HTML 转换为幻灯片后，你将使用 PptxGenJS 添加动态内容，如图表、图像和其他元素。
...
**大多数图表所需**：使用 `catAxisTitle`（类别）和 `valAxisTitle`（值）的轴标签。

**图表数据格式：**
- 对于简单的条形图/折线图使用**带所有标签的单个系列**
- 每个系列创建一个单独的图例条目
- 标签数组定义 X 轴值
...
**关键**：饼图需要**单个数据系列**，在 `labels` 数组中包含所有类别，在 `values` 数组中包含相应的值。
...
**将图表颜色与你选择的设计调色板对齐**，确保数据可视化有足够的对比度和区分度。调整颜色以实现：
- 相邻系列之间的强对比度
- 在幻灯片背景上的可读性
- 可访问性（避免仅使用红绿组合）
