...
name: mcp-builder
name_chn: MCP 构建器
description: 用于创建高质量 MCP(模型上下文协议)服务器的指南,使 LLM 能够通过精心设计的工具与外部服务交互。当构建 MCP 服务器以集成外部 API 或服务时使用,无论是使用 Python(FastMCP)还是 Node/TypeScript(MCP SDK)。
...
要创建高质量的 MCP(模型上下文协议)服务器,使 LLM 能够有效地与外部服务交互,可使用此技能。MCP 服务器提供工具,允许 LLM 访问外部服务和 API。MCP 服务器的质量取决于它如何使 LLM 能够使用所提供的工具完成实际任务。
...
**要收集全面信息,根据需要使用网络搜索和 WebFetch 工具。**
...
- 规划大规模使用(数千用户/资源)
- 实现字符限制和截断策略(例如,25,000 token)

**错误处理策略:**
- 规划优雅的失败模式
- 设计清晰、可操作、对 LLM 友好的自然语言错误消息,提示进一步行动
- 考虑速率限制和超时场景
- 处理身份验证和授权错误
...
现在有了全面的计划,开始按照特定语言的最佳实践进行实现。

#### 2.1 设置项目结构

**对于 Python:**
- 创建单个 `.py` 文件,或者如果复杂则组织成模块(参见 [🐍 Python 指南](./reference/python_mcp_server.md))
...
**重要:** MCP 服务器是长时间运行的进程,通过 stdio/stdin 或 sse/http 等待请求。直接在主进程中运行它们(例如,`python server.py` 或 `node dist/index.js`)将导致进程无限期挂起。
...
评估测试 LLM 是否可以有效使用 MCP 服务器来回答现实、复杂的问题。
