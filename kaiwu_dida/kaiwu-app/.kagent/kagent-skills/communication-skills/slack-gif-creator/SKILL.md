...
name_chn: Slack GIF 创建器
description: 用于创建针对 Slack 优化的动画 GIF 的工具包,具有大小约束验证器和可组合动画基元。当用户根据"为我制作一个 X 做 Y 的 Slack GIF"等描述请求 Slack 的动画 GIF 或表情符号动画时使用此技能。
license: Complete terms in LICENSE.txt
---

# Slack GIF 创建器 - 灵活工具包

用于创建针对 Slack 优化的动画 GIF 的工具包。提供 Slack 约束的验证器、可组合的动画基元和可选的辅助工具。**根据需要应用这些工具以实现创意愿景。**

## Slack 的要求

Slack 对 GIF 有特定要求,具体取决于其用途:
...
2. **动画基元** - 用于运动的可组合构建块(摇晃、弹跳、移动、万花筒)
...
这些是用于常见需求的可选辅助工具。**根据需要使用、修改或替换为自定义实现。**
...
对于像表情符号这样的小型 GIF,文本可读性具有挑战性。常见解决方案涉及添加轮廓:
...
要实现自定义文本渲染,使用 PIL 的 `ImageDraw.text()`,这对于较大的 GIF 很有效。
...
可用的缓动:`linear`、`ease_in`、`ease_out`、`ease_in_out`、`bounce_out`、`elastic_out`、`back_out`(过冲),以及 `core/easing.py` 中的更多。
...
此工具包提供构建块,而不是严格的配方。要处理 GIF 请求:

1. **理解创意愿景** - 应该发生什么?心情是什么?
2. **设计动画** - 将其分解为阶段(预期、动作、反应)
3. **根据需要应用基元** - 摇晃、弹跳、移动、效果 - 自由混合
4. **验证约束** - 检查文件大小,特别是表情符号 GIF
5. **必要时迭代** - 如果超过大小限制,减少帧数/颜色

**目标是在 Slack 的技术约束内实现创作自由。**

## 依赖项

要使用此工具包,仅在尚未安装的情况下安装这些依赖项:
